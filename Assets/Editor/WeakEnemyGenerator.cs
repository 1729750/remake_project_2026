...
// 일회성 에디터 툴: "약한 적" WeakEnemyAI 에셋 하나와, 그 AI를 쓰는 CharacterData(덱은
// 인라인 CardCollection으로 내장) 5개를 Assets/scripts/Enemies/에 생성한다. 이미 존재하는
// 파일이 있으면 덮어쓰지 않고 건너뛴다.
public static class WeakEnemyGenerator
{
    ...
    private static void CreateWeakEnemy(WeakEnemyAI ai, string name, int maxHealth, (string path, int count)[] cardPaths)
    {
        ...
        SerializedProperty cardsProp = dataSo.FindProperty("deck.cards");
